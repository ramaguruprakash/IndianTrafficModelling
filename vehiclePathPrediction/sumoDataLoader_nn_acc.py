...
class SumoDataLoader_nn:
    ...
    def getNNFeaturesFromFrame(self, vehicles, speeds_frame, prev_frame_speeds, frames, infer = False):
            feature_vecs = []
            velocities = []
            coordinates_array = []
            ...
            for vehicle_neighbours,distance in zip(indices,distances):
                ...
                for neighbour,dist in zip(vehicle_neighbours,distance):
                    ...
                    if dist == 0:
                        feature_vec += (speeds_frame[vehicles[neighbour][0]])  ## Own speed of the vehicle
                        ## Add acceleration
                        if vehicles[neighbour][0] not in prev_frame_speeds:
                            feature_vec += [0, 0]
                        else:
                            speed = speeds_frame[vehicles[neighbour][0]]
                            prev_speed = prev_frame_speeds[vehicles[neighbour][0]]
                            feature_vec += [speed[0]-prev_speed[0], speed[1]-prev_speed[1]]   ## Acceleration
                        feature_vec += [vehicles[neighbour][3]]     ## Type
                        ## Done adding acceleration
                    else:
                        #feature_vec.append(dist)
                        feature_vec.append(vehicles[neighbour][1] - vehicles[vehicle_neighbours[0]][1])  ## X distance
                        feature_vec.append(vehicles[neighbour][2] - vehicles[vehicle_neighbours[0]][2])  ## Y distance
                        feature_vec += (speeds_frame[vehicles[neighbour][0]])                            ## Speeds of the vehicles
                        ## Add acceleration
                        if vehicles[neighbour][0] not in prev_frame_speeds:
                            feature_vec += [0, 0]
                        else:
                            ...
                            feature_vec += [speed[0]-prev_speed[0], speed[1]-prev_speed[1]]   ## Acceleration
                        feature_vec += [vehicles[neighbour][3]]                               ## Type
                feature_vec += [-1]*(6*(self.number_of_neighbours+1) - 1 - len(feature_vec))  ## remaining adding -1
    ...
    def loadData(self, sumo_output_file):
        ...
        for frame in frames.keys():
            ...
            vehicles = frames[frame]
            prev_speeds = {}
            if frame-1 in speeds.keys():
                prev_speeds = speeds[frame-1]
            feature_vecs, velocities = self.getNNFeaturesFromFrame(vehicles, speeds[frame], prev_speeds, frames)
            Data_X += feature_vecs
            Data_y += velocity
