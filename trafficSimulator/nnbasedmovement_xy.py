...
import numpy as np
import torch
class nnBasedMovement_xy(VehicleBehavior):
	def __init__(self):
		self.vehicleTypes = VehicleTypes()
                self.dataLoader = SumoDataLoader("",0.1,0.1,1,3,'infer', True)
                self.modelpathx = '/Users/gramaguru/ComputerVision/computer-vision/Traffic/IndianTraffic/vehiclePathPrediction/simulation_1000sec_100cars_100-1_21_100_20_0.0001_5_xmodel.pkl'
                self.modelpathy = '/Users/gramaguru/ComputerVision/computer-vision/Traffic/IndianTraffic/vehiclePathPrediction/simulation_1000sec_100cars_100-1_21_100_20_0.0001_5_ymodel.pkl'
	...
	def updatePos(self, grid, curVehicle, vehicles):
	    ...
	    sizeY = shape[1]
            ...
            netx = Model(21,100,1)
            nety = Model(21,100,1)
            netx.load_state_dict(torch.load(self.modelpathx))
            nety.load_state_dict(torch.load(self.modelpathy))
            outputx = netx(Variable(self.getFloatTensorFromNumpy(feature_vec)))
            outputy = nety(Variable(self.getFloatTensorFromNumpy(feature_vec)))
            velocityx = outputx.data.numpy()
            velocityy = outputy.data.numpy()
            print "output from NN : ", velocityx, velocityy
            #posX += int(velocity[0, 0])
            posY -= int(velocityy[0, 0])
            print "======posY====== ", posY
