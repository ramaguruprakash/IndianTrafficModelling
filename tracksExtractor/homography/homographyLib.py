...
##### Not Working need to debug, but not used anywhere so its left the way it is 
...
def transformSetOfPointsAndReturn(pts, H):
	it = np.nditer(pts, flags=['multi_index'])
	while not it.finished:
		x = it[0]
		it.iternext()
...
def drawPoly(img, pts,color):
        cv2.line(img, (int(pts[0][0]),int(pts[0][1])), (int(pts[1][0]),int(pts[1][1])), color, 5)
        cv2.line(img, (int(pts[1][0]),int(pts[1][1])), (int(pts[2][0]),int(pts[2][1])), color, 5)
        cv2.line(img, (int(pts[2][0]),int(pts[2][1])), (int(pts[3][0]),int(pts[3][1])), color, 5)
        cv2.line(img, (int(pts[3][0]),int(pts[3][1])), (int(pts[0][0]),int(pts[0][1])), color, 5)
        return img;
