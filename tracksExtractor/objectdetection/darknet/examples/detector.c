...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        time=clock();
        network_predict(net, X);
        printf("%s: Predicted in %f seconds.  %d \n", input, sec(clock()-time), l.classes);
        get_region_boxes(l, im.w, im.h, net.w, net.h, thresh, probs, boxes, 0, 0, hier_thresh, 1);
        if (nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
	// My Additional code - after non maximal separation
	FILE *fp = fopen("preComputed.txt","a");
	FILE *fp2 = fopen("boxes.txt","w");
	int num = l.w*l.h*l.n;
	for(int i=0; i < num; i++) {
		box b = boxes[i];
		// Print the probability of the classes
		float max_prob = 0.0;
		int max_prob_class = 0;
		for(int j=0;j < 80; j++){
			//printf("%d - %s\n", j, names[j]);  - For getting the class labels
			if(max_prob < probs[i][j]  && probs[i][j] > 0.25 && (j == 0 || j == 1 || j == 2 || j == 3 || j == 5 || j ==7)){
				max_prob = probs[i][j];
				max_prob_class = j;
			}
		}
		if(max_prob > 0.0) {
			printf("box - %d  max probable vehicle is %s with a probability of %f x=%f, y =%f, w=%f, h=%f\n", i, names[max_prob_class], max_prob, b.x*im.w, b.y*im.h, b.w*im.w, b.h*im.h);
			fprintf(fp, "%f %f %f %f %f %s\n",b.x*im.w, b.y*im.h, b.w*im.w, b.h*im.h, max_prob, names[max_prob_class]);
			fprintf(fp2, "%f %f %f %f %f %s\n",b.x*im.w, b.y*im.h, b.w*im.w, b.h*im.h, max_prob, names[max_prob_class]);
		}
	}
	fprintf(fp, "====\n");
	fclose(fp);
	// Done with my additional code
